...
AI Card 不是通用海报生成模块，而是本系统在自动剪辑里用来“替代人工包装重点信息”的视频画面模板。它主要服务两类场景：

- `viewpoint_statement`：观点陈述段落，切整屏大字卡，强化一句核心观点。
- `climax` / `summary`：段落高潮或小结段落，用更强的视觉强调收束内容。

当前毕设默认走“快速模式优先”，因此 AI Card 必须能在本地稳定生成，不依赖通义千问图片模型，也不因为网络、模型额度或生成耗时影响主流程。模型生成图片后续只作为可选增强，不进入默认渲染链路。

#### 输入字段

渲染阶段只读取 `plan.json` 中的结构化信息，不再重新理解字幕语义：

- `main_scene_type = ai_card`：触发整屏 AI Card。
- `card.title`：卡片主标题，优先承载一句核心观点。
- `card.body`：补充说明，建议 1-3 条短句。
- `headline`：没有 `card.title` 时的标题回退。
- `transcript_excerpt`：没有 `card.body` 时的正文回退。
- `narrative_role`：可作为弱标签或样式选择依据，不作为必需显示内容。
- `transition_in = rapid_flash_cut`：段落高潮时允许使用更强强调色和短闪白提示。
...
- 优先适配中文大字展示。
- 使用本地 PIL 模板生成静态画面，再交给 MoviePy 合成为对应时长的视频片段。
- 默认不额外叠加底部字幕，避免整屏大字卡重复展示同一段文本。

#### 视觉要求

- 画面要像视频包装里的“重点观点页”，不要像网页组件截图、后台调试页或 PPT 默认主题。
- 标题必须是第一视觉层级，适合承载 8-22 个中文字的观点句。
- 正文只承担解释作用，最多展示 3 行，超过内容在 `plan.py` 阶段优先拆 scene 或压缩文案。
- 留白要明显，让观众能在 3-6 秒内读完重点。
- 背景、卡片、强调色保持克制，重点是可读和稳定，不追求复杂动画。
- `rapid_flash_cut` 场景可以使用更强强调色，但不能影响正文可读性。

#### 默认取舍

- 优先保证 1280x720 下清楚可读，1080p 下不显得粗糙。
- 优先保证中文字体可用；找不到中文字体时也要能回退渲染，不让任务失败。
- 优先保证批量渲染稳定，不做逐字动画、复杂粒子和大模型生图。
- 颜色和版式先固定在代码里，后续如果前端要提供主题选择，再抽成配置。
- AI Card 的内容质量主要由 `plan.py` 控制，`render.py` 只负责排版、换行、截断和兜底。
...
- 720p 下可读，1080p 下不粗糙。
- 不依赖外部模型或网络也能生成。
- `card` 字段缺失、正文为空、标题过长时仍能输出可用画面。
- 与 `plan.py` 的 Demo 规律一致：观点陈述优先切整屏 AI Card，段落高潮可配合 `rapid_flash_cut`。
...
## 任务 4.5：人物浮窗 / AI Floating Card Overlay

### 目标

AI Floating Card 是轻量悬浮窗，用来在 PPT 或 speaker 画面上补充极短提示信息。它要像视频包装组件，而不是网页卡片截图。

### 实现内容

- 只在 `overlay.type = ai_card` 时出现。
- 默认位置为右下角；如果当前 `main_scene_type = speaker`，自动切到右上角，避免压住讲者悬浮窗。
- 显示：
  - 人物名
  - 出生年月 / 死亡日期 / 一句简介
- 文案极短，标题建议 10-15 字以内；说明文字只保留一行。
- 触发后 0.5 秒自动唤起。
- 停留 3-5 秒，scene 太短时按剩余时长自动压缩。
- 自动淡入、淡出消失。
- 动画曲线使用 ease-in-out。
- 视觉上使用高圆角、渐变背景、毛玻璃质感和环境阴影。
- AI 大模型总结可以放在 `plan.py` 生成 `overlay` 阶段，渲染阶段只做文案截断和兜底。
